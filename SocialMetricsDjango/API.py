...
#                     except Exception as e:
#                         logger.error(f'Error decodificando la respuesta como texto para la URL: {response.url} - {e}')
#                 except Exception as e:
#                     logger.error(f'Error procesando la respuesta para la URL: {response.url} - {e}')
                
#         def __run(playwright: Playwright):
...
#                     'name': 'hlsPlayback',
#                     'value': 'on',
#                     'domain': 'x.com',  # Asegurarse de que el dominio esté correctamente definido
#                     'path': '/',  # Path es la raíz del dominio
#                     'httpOnly': False,  # Opcional, dependiendo de si la cookie debe ser accedida desde JavaScript
#                     'secure': False  # Establecer si la cookie solo se debe enviar sobre HTTPS
#                 }
#             ]
            
#             viewport = {'width': 375, 'height': 667}  # Resolución de iPhone 13

#             # Lanzar el navegador sin cabeza para ver la interfaz
#             browser = playwright.firefox.launch(headless=True)  # headless=False para poder ver el navegador
...
#             # Registramos la función para manejar las respuestas de la página
#             page.on('response', __handle_response)

#             # Abrir la página de Twitter (modificar si es necesario)
#             page.goto(f'https://x.com/joerogan/')  # Aquí puedes poner la URL dinámica que desees

#             page.wait_for_timeout(2000)  # Esperar 2 segundos (ajustar si es necesario)
#             page.wait_for_timeout(500)   # Esperar más tiempo para asegurar que la carga esté completa

#             # Cierra el navegador después de obtener la información
#             browser.close()
...
#         for instructions in aaa["data"]["user"]["result"]["timeline_v2"]["timeline"]["instructions"]:
#             try:
#                 for entries in instructions["entries"]:
#                     try:
#                         print(entries["content"]["itemContent"]["tweet_results"]["result"].keys())
#                     except:
#                         pass
#             except:
#                 pass
